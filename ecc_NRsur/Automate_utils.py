...
def hlm_modes_given_param(q, Mtot, s1x, s1y, s1z, s2x, s2y, s2z, fref, fmin, approximant=lalsim.GetApproximantFromString('SpinTaylorT4'), lmax=4, dist=300, model_name='PN'):
    ...
    for mode in hlmT_lal.keys():
        ...
        #plot that mode
        if (mode == (2,2) or mode == (3,3) or mode == (4,4)):
...
#timeshift choices
   ...
   ###Optimization function ###########
    ...
    idx = np.argmin(Func)
    if (abs(t0_val[idx])>0.1):
       print("|t0|> 0.1 , hybrid is not possible")
       quit()
...
def get_hyb_modes(T_NR, hlm_NR, T_MD, hlm_MD, Angles_NR, Angles_MD, t0, phi0, psi0, T_for_RigRot):
    ...
    for l in range(2, lmax+1):
        for m in range(-l, l+1):
            ...
            if (l == m): 
               plot_hlm(l,m,T_NR, Re_newNR, Im_newNR, t_model, Re_newMD, Im_newMD)
...
def plot_hlm(l,m,t1, Re1, img1, t2, Re2, img2):
    ...
    plt.axvline(x= -0.7,color='orange',label = "Hybrid interval")
    plt.axvline(x= -0.3,color='orange')
    plt.title("Waveforms after Rigid rotation")
    plt.legend()
...
def get_hyb_22mode(T_NR, hlm_NR, T_MD, hlm_MD, Angles_NR, Angles_MD, t0, phi0, psi0, T_for_RigRot):
